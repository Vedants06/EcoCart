...
function EcoCartApp() {
    ...
    return <div className="eco-cart-app">
        <header className="topbar"><a className="brand" href="#top" aria-label="EcoCart home"><span className="brand-mark" aria-hidden="true" /> EcoCart</a><nav aria-label="Main navigation"><a href="#shop">Shop</a><a href="#impact">Your impact</a><a href="#story">Our story</a></nav><button className={`cart-button ${hasScrolled ? 'cart-button-floating' : ''}`} onClick={() => setCartOpen(true)} aria-label="Open cart"><span className="cart-label">Cart</span><span className="cart-count">{Object.values(cart).reduce((sum, quantity) => sum + quantity, 0)}</span></button></header>
        <main id="top">
            ...
            <section className="impact-section" id="impact"><div><p className="eyebrow">THE ECOCART PROMISE</p><h2>Your checkout can<br /><em>grow something.</em></h2><p>Every order rounds up to fund verified local restoration projects. You choose the goods; we make the good go further.</p><button className="text-button" onClick={() => setRoundup(!roundup)}>{roundup ? 'Round-up on' : 'Round-up off'} <span className={`toggle ${roundup ? 'on' : ''}`} /></button></div><div className="impact-preview"><div className="selection-heading"><span className="eyebrow">YOUR SELECTION</span><span>{cartItems.length} {cartItems.length === 1 ? 'item' : 'items'}</span></div><div className="selection-carousel">{cartItems.length ? cartItems.map((product) => <div className="selection-card" key={product.id}><img src={product.image} alt={product.name} /><div><strong>{product.name}</strong><span>{cart[product.id]} × {money.format(product.price)}</span></div></div>) : <div className="selection-empty"><span className="empty-mark">+</span><p>Add an item to see your impact grow.</p><a href="#shop">Browse the collection ↘</a></div>}</div><div className="impact-stats"><div><span className="stat-number">{treesFunded.toFixed(2)}</span><span>trees funded<br />this order</span></div><div><span className="stat-number">{(treesFunded * 20).toFixed(0)}kg</span><span>CO₂ offset<br />estimated</span></div><div><span className="stat-number">100%</span><span>of round-ups<br />go to impact</span></div></div></div></section>
        ...
        <footer><span className="brand"><span className="brand-mark" aria-hidden="true" /> EcoCart</span><span>© 2026 EcoCart Collective</span><span>Made for a lighter tomorrow.</span></footer>
